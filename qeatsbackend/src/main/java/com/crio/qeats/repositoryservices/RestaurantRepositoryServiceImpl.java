...
public class RestaurantRepositoryServiceImpl implements RestaurantRepositoryService {
  ...
  public List<Restaurant> findAllRestaurantsCloseBy(Double latitude,
    ...
    // String geoHashPrefix = GeoHash.geoHashStringWithCharacterPrecision(latitude, longitude, 6);

    // Query query = new Query(Criteria.where("geoHash").regex("^" + Pattern.quote(geoHashPrefix)));

    // List<RestaurantEntity> restaurantEntities = mongoTemplate.find(query, RestaurantEntity.class);

    // List<Restaurant> restaurants = restaurantEntities.stream()
    //     // .filter(restaurant -> isOpenNow(currentTime, restaurant))
    //     .filter(restaurantEntitie -> isRestaurantCloseByAndOpen(restaurantEntitie, currentTime, latitude, longitude, servingRadiusInKms))
    //     .map(restaurantEntity -> modelMapperProvider.get().map(restaurantEntity, Restaurant.class))
    //     .collect(Collectors.toList());
